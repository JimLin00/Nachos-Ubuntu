...
AddrSpace::AddrSpace() {
    ...
    for (int i = 0; i < NumPhysPages; i++) {
        ...
        // Required Features : 
        // 1. Initialize "physicalPage" as 0.
...
{
    /* Lab3 - NachOS - Memory Management - Start */
    // sync frameTable before process-owned pageTable being released
...
AddrSpace::Load(char *fileName) 
{
    ...
    /* Lab3 - NachOS - Memory Management - Start */
    // Adding the ExceptionType variable to handle the exceeption
...
// how big is address space?
    ...
    /* Lab3 - NachOS - Memory Management - Start */
    // Required Features:
    // 1. Check if the virtual page exceeds the physical page limit.
    ...
    // Before you start:
    // 1. You should understand the meaning of each variable, including but not limited to "size", "numPages", "NumPhysPages"
    // Some variables have comments explaining their features where they are declared.
    // 2. The for loop at line 181 can be removed if you implement your own method.
    ...
    // i: virtual page index
    // j: physical page index
    ...
    DEBUG(dbgAddr, "Finish initialization");
...
// Note: this code assumes that virtual address = physical address
    if (noffH.code.size > 0) {
        ...
        int blockSize = PageSize - noffH.code.virtualAddr % PageSize; // 1st block size
        unsigned int virtualAddr = noffH.code.virtualAddr;
        unsigned int physAddr;
        Translate(virtualAddr, &physAddr, 1);
        executable->ReadAt(&(kernel->machine->mainMemory[physAddr]), blockSize, fileOffset);
        ...
        Translate(virtualAddr, &physAddr, 1);
        executable->ReadAt(&(kernel->machine->mainMemory[physAddr]), remainSize, fileOffset);
        virtualAddr += remainSize;
        fileOffset += remainSize;
        ASSERT(virtualAddr == noffH.code.virtualAddr + noffH.code.size);
        ASSERT(fileOffset == noffH.code.inFileAddr + noffH.code.size);
    ...
    if (noffH.initData.size > 0) {
        ...
        cout << "initData size: " << noffH.initData.size << endl;
        int i = 0;
        
        /* Lab3 - NachOS - Memory Management - Start */

        int fileOffset = noffH.initData.inFileAddr;
        int remainSize = noffH.initData.size;
        int blockSize = PageSize - noffH.initData.virtualAddr % PageSize; // 1st block size
        unsigned int virtualAddr = noffH.initData.virtualAddr;
        unsigned int physAddr;
        Translate(virtualAddr, &physAddr, 1);
        executable->ReadAt(&(kernel->machine->mainMemory[physAddr]), blockSize, fileOffset);
        cout << "read #" << ++i << " page, physical address: " << physAddr << endl;
        ...
        virtualAddr += remainSize;
        fileOffset += remainSize;
        ASSERT(virtualAddr == noffH.initData.virtualAddr + noffH.initData.size);
        ASSERT(fileOffset == noffH.initData.inFileAddr + noffH.initData.size);
    ...
    if (noffH.readonlyData.size > 0) {
        ...
        int fileOffset = noffH.readonlyData.inFileAddr;
        int remainSize = noffH.readonlyData.size;
        int blockSize = PageSize - noffH.readonlyData.virtualAddr % PageSize; // 1st block size
        ...
        virtualAddr += remainSize;
        fileOffset += remainSize;
        ASSERT(virtualAddr == noffH.readonlyData.virtualAddr + noffH.readonlyData.size);
        ASSERT(fileOffset == noffH.readonlyData.inFileAddr + noffH.readonlyData.size);
